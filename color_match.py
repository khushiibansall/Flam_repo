...
class ColorMatcher:
    ...
    def extract_reference_colors(self, background, mask, sample_size=50):
        """
        Extract representative colors from background areas adjacent to the person
        """
        # Create edge zones around the person for color sampling
        kernel = np.ones((sample_size, sample_size), np.uint8)
        dilated_mask = cv2.dilate(mask, kernel, iterations=1)
        edge_zone = dilated_mask - mask
        
        # Sample colors from edge zone
        edge_pixels = background[edge_zone > 0]
        if len(edge_pixels) == 0:
            # Fallback: sample from entire background
    ...
    def advanced_color_transfer(self, source, target, mask=None, preserve_luminance=0.3):
        """
        Enhanced color transfer with luminance preservation and local adaptation
        """
        # Convert to LAB color space for perceptually uniform color matching
        source_lab = cv2.cvtColor(source, cv2.COLOR_BGR2LAB).astype(np.float32)
        target_lab = cv2.cvtColor(target, cv2.COLOR_BGR2LAB).astype(np.float32)
        
        # If mask provided, use only valid regions for statistics
        if mask is not None:
        ...
        for i in range(3):
            ...
            # Normalize source, scale to target distribution
            ...
            # Preserve some original luminance (L channel)
    ...
    def local_color_adaptation(self, person, background, mask, adaptation_strength=0.5):
        """
        Apply local color adaptation based on surrounding background colors
        """
        # Create distance transform to weight adaptation by proximity to edges
        dist_transform = cv2.distanceTransform(mask, cv2.DIST_L2, 5)
        max_dist = np.max(dist_transform)
        
        if max_dist > 0:
            # Normalize distance weights (stronger adaptation at edges)
    ...
    def match_histogram_selective(self, source, reference, mask=None, channels='all'):
        """
        Selective histogram matching for specific color channels
        """
        if channels == 'all':
            channels = [0, 1, 2]
...
def create_seamless_blend(person, background, mask, blend_mode='multiband'):
    """
    Create seamless blending between person and background
    """
...
def multiband_blending(person, background, mask, levels=4):
    ...
    # Build pyramids for person and background
...
def complete_color_matching_pipeline():
    """
    Complete pipeline for color matching and blending
    """
    # Load images
    try:
        ...
        # Resize background to match person
        ...
        # Step 3: Selective histogram matching
        print("Step 3: Matching color histograms...")
        histogram_matched = color_matcher.match_histogram_selective(
            locally_adapted, bg_resized, mask, channels=[1, 2]  # Only a,b channels
        )
        
        # Step 4: Enhance lighting consistency
        ...
        # Step 6: Final composite with soft edges
